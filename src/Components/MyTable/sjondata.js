...
const parseAccounts = (data) => {
    return Object.keys(data).map((accountId) => {
      const account = data[accountId];
      const children = account.children ? parseAccounts(account.children) : [];
      return { accountId, ...account, children };
    });
  };
  
  // Convert JSON data to hierarchical format
export  const accountsData = parseAccounts(JsonData);
